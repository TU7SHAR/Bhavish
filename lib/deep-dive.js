...
Return ONLY valid JSON in EXACTLY this shape:
{
  ...
  try {
    ...
    console.error("Deep-dive parse error:", e.message);
    throw new Error("Failed to generate deep-dive. Please try again.");
  }

  const sections = Array.isArray(parsed?.sections) ? parsed.sections : [];
  // Expect 7 deep-dive + 1 roadmap = 8. Accept >= 6 to tolerate minor variance.
  if (sections.length < 6) {
    throw new Error("Incomplete deep-dive generated.");
  }
