 ...
 */
export async function sendPurchaseEvent({ reportId, value, currency = "INR", email, planTier, eventSourceUrl, fbp, fbc } = {}) {
  ...
  // fbp/fbc are the Meta browser cookies (_fbp/_fbc). Sending them on the
  // SERVER event so it matches the browser Pixel event → reliable dedup.
  // These are identifiers, NOT PII, so they are sent as-is (not hashed).
