...
// Navamsa (D9) sign from a sidereal longitude.
// The 360° zodiac is divided into 108 parts of 3°20' each, cycling through
// the 12 signs continuously starting from Aries. This continuous mapping
// exactly reproduces the classical navamsa rules (movable signs start from
// self, fixed from the 9th, dual from the 5th) — verified for all three.
// The LLM cannot reliably do this division, so we compute it for it.
function getNavamsaSign(siderealLongitude) {
  ...
  if (lon < 0) lon += 360;
  const navIndex = Math.floor(lon / (30 / 9)) % 12;
  return signs[navIndex];
}
 ...
 */
// English ordinal suffix (1 -> "st", 2 -> "nd", 9 -> "th", etc.)
function ordinal(n) {
  const s = ["th", "st", "nd", "rd"];
  const v = n % 100;
...
// Manglik (Mangal / Kuja Dosha) status. Classical rule: Mars in the
// 1st, 2nd, 4th, 7th, 8th, or 12th house counted from the Lagna, the Moon,
// OR Venus. This is a deterministic rule, so we compute it rather than let
// the LLM decide — it wrongly called 9th-house Mars "Manglik" in generated
// reports. Returns the verdict, the house from each reference, and a summary.
function computeManglik(planets, ascSignIndex) {
  const doshaHouses = [1, 2, 4, 7, 8, 12];
  const marsSign = planets.Mars.signIndex;
  const houseFrom = (refSignIndex) => ((marsSign - refSignIndex + 12) % 12) + 1;

  const refs = {
    Lagna: houseFrom(ascSignIndex),
    Moon: houseFrom(planets.Moon.signIndex),
    Venus: houseFrom(planets.Venus.signIndex),
  };

  // TRADITIONAL Manglik = Mars in a dosha house from the LAGNA or the MOON.
  // These are the primary references every classical text uses.
  const traditionalRefs = [];
  if (doshaHouses.includes(refs.Lagna)) traditionalRefs.push(`Lagna (Mars in the ${ordinal(refs.Lagna)} house)`);
  if (doshaHouses.includes(refs.Moon)) traditionalRefs.push(`Moon (Mars in the ${ordinal(refs.Moon)} house)`);

  // VENUS-based is a SECONDARY, lesser condition used by some traditions. It is
  // NOT the same as primary Manglik Dosha and must not be reported as such — a
  // chart clean from Lagna AND Moon but flagged only from Venus should be shown
  // as a "secondary Venus-Manglik indication", never a flat "You are Manglik".
  const venusTriggered = doshaHouses.includes(refs.Venus);

  const isTraditionalManglik = traditionalRefs.length > 0;
  const isSecondaryOnly = !isTraditionalManglik && venusTriggered;
  // Kept for backward compatibility (the free tool + prior callers read this),
  // but note isManglik is TRUE for the secondary case too — always check
  // `category` for the correct severity.
  const isManglik = isTraditionalManglik || venusTriggered;

  // category: "none" | "traditional" | "secondary_venus"
  const category = isTraditionalManglik ? "traditional" : venusTriggered ? "secondary_venus" : "none";

  const positions = `Mars is in the ${ordinal(refs.Lagna)} house from Lagna, ${ordinal(refs.Moon)} from the Moon, and ${ordinal(refs.Venus)} from Venus`;

  let summary;
  if (isTraditionalManglik) {
    const venusNote = venusTriggered ? " A secondary Venus-based indication is also present." : "";
    summary = `Manglik (Mangal Dosha): YES, TRADITIONAL — triggered from ${traditionalRefs.join(" and ")}.${venusNote} ${positions}. (Manglik houses are 1, 2, 4, 7, 8, 12.)`;
  } else if (isSecondaryOnly) {
    summary =
      `Manglik (Mangal Dosha): SECONDARY (Venus-based) ONLY — NOT traditional Manglik. ` +
      `Mars is in the ${ordinal(refs.Venus)} house from Venus, which some traditions treat as a secondary, less-severe Venus-Manglik indication. ` +
      `Mars is NOT in a Manglik house from the Lagna (${ordinal(refs.Lagna)}) or the Moon (${ordinal(refs.Moon)}), so this is NOT a standard/primary Manglik Dosha. ` +
      `Present it as a "secondary condition", do NOT say "you are Manglik" flatly, and do NOT alarm the reader or demand strict Manglik matching/major remedies. (Manglik houses are 1, 2, 4, 7, 8, 12.)`;
  } else {
    summary = `Manglik (Mangal Dosha): NO. ${positions} — none of these are Manglik houses (1, 2, 4, 7, 8, 12).`;
  }

  return {
    isManglik,
    isTraditionalManglik,
    isSecondaryOnly,
    category,
    references: refs,
    summary,
...
// Compute the well-defined classical yogas deterministically so the LLM never
// has to guess them (it was giving contradictory Kaal Sarp / yoga verdicts
// across regenerations of the same chart). Covers: Kaal Sarp, Budhaditya,
// Gajakesari, Chandra-Mangala, the 5 Pancha Mahapurusha yogas, and Neecha Bhanga.
function computeYogas(planets, ascSignIndex) {
  const KENDRA = [1, 4, 7, 10];
  const signLord = { 1: "Mars", 2: "Venus", 3: "Mercury", 4: "Moon", 5: "Sun", 6: "Mercury", 7: "Venus", 8: "Mars", 9: "Jupiter", 10: "Saturn", 11: "Saturn", 12: "Jupiter" };
  const exaltedPlanetInSign = { 1: "Sun", 2: "Moon", 4: "Jupiter", 6: "Mercury", 7: "Saturn", 10: "Mars", 12: "Venus" };
  const ownSigns = { Sun: [5], Moon: [4], Mars: [1, 8], Mercury: [3, 6], Jupiter: [9, 12], Venus: [2, 7], Saturn: [10, 11] };
  const exaltSign = { Sun: 1, Moon: 2, Mars: 10, Mercury: 6, Jupiter: 4, Venus: 12, Saturn: 7 };
  const houseFrom = (fromSignIdx, targetSignIdx) => ((targetSignIdx - fromSignIdx + 12) % 12) + 1;

  const moonSign = planets.Moon.signIndex;
  const seven = ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"];
  const yogas = [];

  // --- Kaal Sarp Yoga: are all 7 planets hemmed on one side of the Rahu-Ketu axis? ---
  const rahu = parseFloat(planets.Rahu.fullDegree);
  const ketu = parseFloat(planets.Ketu.fullDegree);
  const lo = Math.min(rahu, ketu), hi = Math.max(rahu, ketu);
  let inside = 0, outside = 0;
  for (const b of seven) {
    const lon = parseFloat(planets[b].fullDegree);
    if (lon > lo && lon < hi) inside++; else outside++;
  }
  const kaalSarpPresent = inside === 0 || outside === 0;
  const ksTypes = { 1: "Anant", 2: "Kulik", 3: "Vasuki", 4: "Shankhpal", 5: "Padma", 6: "Mahapadma", 7: "Takshak", 8: "Karkotak", 9: "Shankhachud", 10: "Ghatak", 11: "Vishdhar", 12: "Sheshnag" };
  const kaalSarp = { present: kaalSarpPresent, type: kaalSarpPresent ? ksTypes[planets.Rahu.house] : null };

  // --- Budhaditya Yoga: Sun + Mercury in the same sign ---
  if (planets.Sun.signIndex === planets.Mercury.signIndex) {
    yogas.push(`Budhaditya Yoga (Sun-Mercury conjunct in ${planets.Sun.sign}, house ${planets.Sun.house}) — sharp intellect and articulate communication`);
  }

  // --- Gajakesari Yoga: Jupiter in a kendra (1/4/7/10) from the Moon ---
  if (KENDRA.includes(houseFrom(moonSign, planets.Jupiter.signIndex))) {
    yogas.push(`Gajakesari Yoga (Jupiter in a kendra from the Moon) — wisdom, reputation and lasting fame`);
  }

  // --- Chandra-Mangala Yoga: Moon + Mars in the same sign ---
  if (planets.Moon.signIndex === planets.Mars.signIndex) {
    yogas.push(`Chandra-Mangala Yoga (Moon-Mars conjunct in ${planets.Moon.sign}) — drive and wealth through self-effort`);
  }

  // --- Pancha Mahapurusha Yogas: planet in own/exaltation sign AND in a kendra from Lagna ---
  ...
  for (const [planet, yogaName] of Object.entries(mahapurusha)) {
    ...
    const ownOrExalt = ownSigns[planet].includes(p.signIndex) || exaltSign[planet] === p.signIndex;
    if (ownOrExalt && KENDRA.includes(p.house)) {
      yogas.push(`${yogaName} Yoga / Pancha Mahapurusha (${planet} strong in ${p.sign} in kendra house ${p.house}) — a rare yoga granting authority and strength`);
    }
  }

  // --- Neecha Bhanga Raja Yoga: cancellation of a debilitated planet's weakness ---
  for (const planet of seven) {
    const p = planets[planet];
    if (!p.dignity || !p.dignity.includes("Debilitated")) continue;
    const reasons = [];
    const lord = signLord[p.signIndex];
    if (KENDRA.includes(planets[lord].house) || KENDRA.includes(houseFrom(moonSign, planets[lord].signIndex))) {
      reasons.push(`its dispositor ${lord} sits in a kendra`);
    }
    const exP = exaltedPlanetInSign[p.signIndex];
    if (exP && planets[exP] && (KENDRA.includes(planets[exP].house) || KENDRA.includes(houseFrom(moonSign, planets[exP].signIndex)))) {
      reasons.push(`${exP} (which exalts in ${p.sign}) sits in a kendra`);
    }
    if (reasons.length > 0) {
      yogas.push(`Neecha Bhanga Raja Yoga (${planet} debilitated in ${p.sign}, but cancelled because ${reasons.join(" and ")}) — early struggles convert into eventual rise`);
    }
  }

  const summary =
    (kaalSarp.present
      ? `Kaal Sarp Yoga: PRESENT (${kaalSarp.type} type — all seven planets are hemmed on one side of the Rahu-Ketu axis).`
      : `Kaal Sarp Yoga: ABSENT (the seven planets fall on BOTH sides of the Rahu-Ketu axis, so there is NO Kaal Sarp Yoga — do not claim even a partial one).`) +
    (yogas.length > 0
      ? ` Yogas present in this chart: ${yogas.join("; ")}.`
      : ` No other major classical yogas are present beyond ordinary placements.`);

  return { kaalSarp, list: yogas, summary };
}

export function calculateBirthChart({ dateOfBirth, timeOfBirth, latitude, longitude, timezoneOffsetMinutes }) {
  // Parse date and time
  ...
  // Priority:
  //   1. Explicit timezoneOffsetMinutes (e.g. 330 for IST, -300 for US Eastern)
  //   2. Geographic timezone from longitude (longitude / 15 * 60).
  //      This gives the LOCAL MEAN TIME (LMT) offset, which is astronomically
  //      correct and what Vedic astrology traditionally uses. It differs from
  //      civil time by at most ~30 min (timezone boundary effects), but is
  //      accurate enough for planetary positions (Moon moves ~0.5°/hour, so
  //      a 30-min error ≈ 0.25° — less than the ayanamsa uncertainty).
  //
  // Previously this hardcoded IST (+5:30) which broke charts for anyone
  // born outside India.
  const offsetMinutes = timezoneOffsetMinutes != null
    ? timezoneOffsetMinutes
    : Math.round(longitude / 15 * 60); // LMT: 1 hour per 15° of longitude

  const totalLocalMinutes = hours * 60 + minutes;
  const totalUtcMinutes = totalLocalMinutes - offsetMinutes;

  // Handle day boundary wraparound (e.g., 00:30 local with +60 offset → previous day 23:30 UTC)
  let dayAdjust = 0;
  let adjUtcMinutes = totalUtcMinutes;
  if (adjUtcMinutes < 0) {
    adjUtcMinutes += 24 * 60;
    dayAdjust = -1;
  } else if (adjUtcMinutes >= 24 * 60) {
    adjUtcMinutes -= 24 * 60;
    dayAdjust = 1;
  }

  const utcHours = Math.floor(adjUtcMinutes / 60);
  const utcMinutes = adjUtcMinutes % 60;
  const birthDate = new Date(Date.UTC(year, month - 1, day + dayAdjust, utcHours, utcMinutes, 0));
  ...
  for (const [name, data] of Object.entries(planets)) {
    ...
    // Navamsa (D9) sign — computed so the LLM never has to guess it.
    data.navamsa = getNavamsaSign(parseFloat(data.sidereal));
  }

  // Manglik / Mangal Dosha — computed (Mars in 1/2/4/7/8/12 from Lagna, Moon, Venus)
  const manglik = computeManglik(planets, ascSignIndex);

  // Classical yogas (Kaal Sarp, Budhaditya, Gajakesari, Chandra-Mangala,
  // Pancha Mahapurusha, Neecha Bhanga) — computed so Section 15 is deterministic
  const yogas = computeYogas(planets, ascSignIndex);
  ...
  return {
    ascendant: {
      ...
      navamsa: getNavamsaSign(ascendantLong),
    },
    planets,
    nakshatra,
    rashi,
