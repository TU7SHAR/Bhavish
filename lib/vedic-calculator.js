...
export function calculateBirthChart({ dateOfBirth, timeOfBirth, latitude, longitude }) {
  ...
  for (const [name, body] of Object.entries(planetBodies)) {
    ...
    if (body === Astronomy.Body.Sun) {
      ecliptic = Astronomy.SunPosition(astroDate);
    } else {
      ecliptic = Astronomy.EclipticGeoMoon(astroDate);
      if (body !== Astronomy.Body.Moon) {
        const equatorial = Astronomy.Equator(body, astroDate, null, true, true);
        ecliptic = Astronomy.Ecliptic(equatorial.vec);
      }
    }
  ...
  for (const [name, data] of Object.entries(planets)) {
    data.house = getHouse(parseFloat(data.sidereal || data.fullDegree), ascendantLong);
  }
