...
function computeManglik(planets, ascSignIndex) {
  ...
  const triggeredFrom = Object.entries(refs)
    .filter(([, house]) => doshaHouses.includes(house))
    .map(([ref, house]) => `${ref} (Mars in the ${ordinal(house)} house)`);

  const isManglik = triggeredFrom.length > 0;
  ...
  const summary = isManglik
    ? `Manglik (Mangal Dosha): YES — triggered from ${triggeredFrom.join(", ")}. ${positions}. (Manglik houses are 1, 2, 4, 7, 8, 12.)`
    : `Manglik (Mangal Dosha): NO. ${positions} — none of these are Manglik houses (1, 2, 4, 7, 8, 12).`;

  return { isManglik, references: refs, summary };
}
