...
Use ${name}'s name. Mix Hindi/Sanskrit with English. Return ONLY valid JSON.`;
  ...
  // Try up to 2 times — flash-lite can occasionally truncate long reports.
  ...
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
    try {
      ...
      try {
        ...
        console.error(`Report parse error (attempt ${attempt}):`, parseError.message);
        lastGenError = new Error("Failed to generate full report. Please try again.");
        continue;
      }

      // Quality gate: enough sections, and the paid guidance section must exist if bought.
      const minSections = effectiveTier === "essential" ? 8 : 12;
      ...
      if (
        ...
        console.warn(`Quality gate failed (attempt ${attempt}): got ${reportData?.sections?.length || 0} sections, need ${minSections}${includeGuidance ? " + guidance" : ""}`);
        lastGenError = new Error("Incomplete report. Please try again.");
        reportData = null;
        continue;
      }

      // Passed quality gate — break out of retry loop
      break;
    } catch (err) {
      lastGenError = err;
  ...
  if (!reportData) {
    throw lastGenError || new Error("Failed to generate report after retries.");
  }
