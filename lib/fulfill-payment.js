...
import { logEvent, logWarn, logError } from "./ops-log.js";
...
export async function fulfillPayment({ reportId, paymentId, planId, includeGuidance, includeBump = false, source = "server" }) {
  ...
  if (hasRealReport && isHistoricalSale) {
    ...
    await logEvent({
      event: "fulfill.legacy_settled",
      source: `fulfill:${source}`,
      reportId,
      message: "historical sale already had a report — settled, not re-sent",
      meta: { paidAt: report.paid_at || null, backfilled: Object.keys(backfill).join(",") || "none" },
    });
  ...
  try {
    ...
    if (!justPaid) {
      ...
      if (!isFresh) {
        ...
        await logEvent({
          event: "meta.purchase_suppressed_stale",
          source: "fulfillPayment",
          reportId: report.report_id,
          message: "stale sale — not reported to Meta as a new purchase",
          meta: { paidAt: report?.paid_at || null },
        });
        return;
      }
    }
    ...
    await logEvent({
      ...
      meta: {
        eventId: `purchase_${report.report_id}`,
        value,
        skipped: result?.skipped ?? null,
        sendError: result?.error ?? null,
        hadFbp: !!report.meta_fbp,
        hadFbc: !!report.meta_fbc,
      },
    });
  } catch (e) {
 ...
 * @param {boolean} [options.notifyOwnerOfSale=true] - when false, the customer
 *   report email is still sent, but the owner "New Sale" notification is
 *   suppressed. Used for GIFTED reports (see /api/admin/gift, type "essential"),
 *   which are not sales and must not trigger a sale alert. Defaults to true so
 *   every real payment path keeps notifying.
 * @returns {Promise<{emailed:boolean, claimed:boolean, skipped?:string}>}
 */
export async function deliverReport(supabase, report, generated, options = {}) {
  const { notifyOwnerOfSale = true } = options;
  ...
  // 'gifted' is allowed alongside 'paid'/'founder': an admin-gifted report is an
  // intentional free send (see /api/admin/gift, type "essential"). It is still
  // excluded from revenue because payment_status !== "paid".
  const payStatus = (report.payment_status || "").toLowerCase();
  if (payStatus && payStatus !== "paid" && payStatus !== "founder" && payStatus !== "gifted") {
    console.warn(
    ...
    await logWarn({
      ...
      source: "deliverReport",
      reportId: report.report_id,
      message: `refused: payment_status="${report.payment_status}"`,
      meta: { paymentStatus: report.payment_status || null },
    });
  ...
  // The single most important customer-facing outcome in the whole system:
  // did the person who paid actually receive the thing they bought?
  await logEvent({
    event: emailed ? "report.delivered" : "report.delivery_failed",
    level: emailed ? "info" : "error",
    source: "deliverReport",
    reportId: report.report_id,
    message: emailed ? `emailed ${tier} report` : "email send FAILED — claim released for retry",
    meta: {
      tier,
      to: report.email,
      guidance: guidanceOn,
      sections: generated?.sections?.length ?? null,
    },
  });

  // Owner notification (best-effort; not part of the atomic claim).
  // Owner "New Sale" notification — SKIPPED for gifted reports (not a sale).
  if (notifyOwnerOfSale) {
    await notifyOwner(
      report,
      report.payment_id,
