  ...
  if (reportCompleted) {
    ...
    if (!emailAlreadySent && report.email && report.email.trim()) {
      if (!isMaster || fresh?.deep_dive_status === "completed") {
        await sendReportEmail({ report, generated: { summary: report.summary, sections: fresh.sections }, guidanceOn });
    ...
    if (checkRow?.report_status === "generating" || checkRow?.report_status === "completed") {
      ...
      if (checkRow?.report_status === "completed") {
        if (!emailAlreadySent && report.email) {
          ...
          if (fullRow) {
            await sendReportEmail({ report, generated: fullRow, guidanceOn });
            await supabase.from("reports").update({ email_sent_at: new Date().toISOString() }).eq("report_id", reportId);
          }
        }
      }
