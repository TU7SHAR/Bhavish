...
export async function fulfillPayment({ reportId, paymentId, planId, includeGuidance, includeBump = false, source = "server" }) {
  ...
  // 2. Idempotency — if already fully done or being generated, do nothing.
  const alreadyPaid = report.payment_status === "paid";
  const alreadyDelivered = report.report_status === "completed";
  const alreadyGenerating = report.report_status === "generating";
  if (alreadyPaid && alreadyDelivered) {
    return { status: "already_done", reportId };
  }
  if (alreadyPaid && alreadyGenerating) {
    // Another process (browser) already claimed the generation slot.
  ...
  //    Use atomic claim so browser and webhook don't double-generate.
  ...
  // Atomic claim — if the browser already claimed generation, skip.
  let claimed = false;
  try {
    const { data: claimResult } = await supabase.rpc("claim_report_generation", { p_report_id: reportId });
    claimed = !!claimResult;
  } catch {
    // RPC not available — fallback check
    const freshReport = await supabase.from("reports").select("report_status").eq("report_id", reportId).single();
    if (freshReport.data?.report_status === "generating" || freshReport.data?.report_status === "completed") {
      return { status: "already_generating", reportId };
    }
    // Claim with simple update
    const { data: rows } = await supabase
      .from("reports").update({ report_status: "generating" })
      .eq("report_id", reportId).neq("report_status", "generating").neq("report_status", "completed")
      .select("report_id");
  ...
  try {
    ...
    // 6. Email the report to the customer.
    //    For MASTER tier: hold the email until the deep-dive completes (the
    //    deep-dive endpoint will send the final email with all 30 sections).
    //    For Essential/Premium: email immediately — the report is complete.
    if (report.email && report.email.trim() && !plan.deepDive) {
      await sendReportEmail({ report, generated, guidanceOn });
    }

    // 7. Master tier: trigger the concern-specific deep-dive as a SEPARATE job.
    //    The deep-dive endpoint will email the customer after completing.
    if (plan.deepDive) {
