 ...
 * @param {boolean} [options.notifyOwnerOfSale=true] - set false when delivering a
 *   historical backlog row, so the owner isn't emailed "New Sale!" for a months-old
 *   purchase. The customer email is still sent.
...
export async function deliverReport(supabase, report, generated, options = {}) {
  ...
  // standing between a wrongly-flagged row and the product being emailed out
  // for free — and it did not check at all. Now it does.
  // 'founder' is allowed: those are intentional free founder-member reports.
  const payStatus = (report.payment_status || "").toLowerCase();
  if (payStatus !== "paid" && payStatus !== "founder") {
    console.warn(
  ...
  if (!claimed) return { emailed: false, claimed: false, skipped: "already_delivered" }; // someone else already delivered
  ...
  // Suppressed for backlog deliveries so draining months-old undelivered
  // purchases doesn't spam the owner with "New Sale!" for old money.
