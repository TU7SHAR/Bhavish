...
// `request` is whatever generateContent accepts — a plain prompt string OR a
// full request object ({ contents, systemInstruction, generationConfig, ... }).
// We forward it unchanged, so callers using systemInstruction (e.g. draft-reply)
// get the IDENTICAL request on the primary and every fallback — no discrepancy.
 ...
 * @param {string|object} prompt  a prompt string OR a full generateContent
 *                                 request object (contents/systemInstruction/…).
 *                                 Forwarded byte-for-byte to every model, so
 *                                 the primary and all fallbacks see the exact
 *                                 same input — no prompt discrepancy.
